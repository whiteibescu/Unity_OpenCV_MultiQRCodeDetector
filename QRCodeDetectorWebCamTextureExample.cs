    ...
    public class QRCodeDetectorWebCamTextureExample : MonoBehaviour
    {
        ...
        void Update()
        {
            if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
            {
                ...
                List<Mat> straight_qrcode = new List<Mat>();

                QRCodeDetector detector = new QRCodeDetector();

                bool result = detector.detectAndDecodeMulti(grayMat, decoded_info, points, straight_qrcode);
                ...
                if (result)
                {
                    ...
                    try
                    {
                        for (int i = 0; i < points.rows(); i++)
                        {
                            ...
                            Imgproc.putText(rgbaMat, decoded_info[i], new Point(points_arr[0], points_arr[1]), Imgproc.FONT_HERSHEY_SIMPLEX, 0.7, new Scalar(255, 255, 255, 255), 2, Imgproc.LINE_AA, false);
                        }
                    }
                    catch
                    {
